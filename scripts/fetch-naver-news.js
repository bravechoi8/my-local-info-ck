...
const BLOCK_KEYWORDS = [
  ...
  '유산', '사산', '해산비', '참전유공자', '독립유공자', '농촌맞춤형', '재능나눔', '농촌', '농어촌', '농촌형', '농어촌형', '농어촌버스', '농촌보육', '농촌돌봄',
  ...
  // 비인기 키워드 추가
  '연탄', '보일러', '연탄보일러', '연탄쿠폰',
  '장애학생', '장애 학생', '특수교육', '장애아동', '특수학교', '장애인 체육', '체육 장학금',
...
1. **뉴스 분석의 독창성 확보**: 단순히 뉴스 내용을 그대로 복사 나열하지 마세요. 해당 뉴스가 발생하게 된 원인(사회경제적 맥락, 제도 변화 등)에 대해 최소 2개 이상의 별도 문단으로 심층 분석해 줘.
2. **이슈에 대한 대중의 시선 및 시장 파급 효과 분석**: 이 소식을 들은 대중들의 흥미로운 반응, 향후 관련 업계나 일상에 미칠 영향이나 변화 전망을 시각적 분석과 함께 설명해 줘.
3. **독자 지향적 행동 촉구(Action Plan)**: 이 뉴스를 통해 독자가 실제로 취해야 할 조치나 혜택(예: "관련 신청이 시작되는 날 알림 설정해두기", "이런 상황일 때는 기존 계약서 다시 점검해보기" 등)을 요약 꿀팁으로 강제 구성해 줘.
4. **글 본문 중간 링크 삽입 규칙**: 뉴스를 다루면서 관련된 정부·공식 홈페이지나 공인 포털(예: 복권위원회 공식 홈페이지, 금융감독원 파인 등)의 주소가 있다면 본문 중간 맥락에 자연스러운 링크([사이트명](URL))를 최소 1~2개 이상 삽입해 줘. 단, 주제와 100% 일치하는 공식 방송/뉴스 영상이 없는 경우에는 억지로 무관한 유튜브 영상 링크를 넣지 말고 오직 신뢰도 높은 본문 텍스트와 표, 공식 누리집 링크만으로 깔끔하게 작성해 줘.

[본문 분량 및 구성]
글의 분량은 무조건 **최소 2000자에서 2500자 내외**로 아주 풍부하고 상세하게 작성해야 해. 단순히 얕은 수준의 기사 받아쓰기를 지양하고, 독자들에게 실질적으로 전문적인 배경 지식과 도움, 흥미로운 스토리를 선사해야 해. 억지로 글자 수를 늘리는 게 아니라, 상세한 구조적 해설(H2, H3 단락 구분), 관련 FAQ 코너(최소 3개 질문 이상)를 반드시 곁들여 줘.
...
만약 키워드가 '로또 당첨번호'인 경우, 사람들의 큰 관심을 끌 수 있는 로또 당첨번호 안내 포스팅(예: '1120회 로또 1등 당첨번호 명당 어디? 실수령액까지 완벽 요약')으로 친근하고 호기심 있게 작성해줘. 1등 번호, 보너스 번호뿐만 아니라, 특히 **전국의 모든 1등 당첨 판매점의 [상호명, 상세 도로명 주소, 선택 방식(자동/수동/반자동)] 목록**을 참고 기사 정보에서 상세하게 수집하여 깔끔하고 가독성 높은 표(Table) 형식으로 하나도 빠질없이 꼼꼼하게 정리해서 안내해줘야 해. 또한, 1등 당첨금(세전)에 대한 예상 세금(3억 이하 22%, 3억 초과 33% 원천징수 세율 적용) 계산법과 당첨 게임수 등을 바탕으로 **실수령액(세후) 예상 금액을 표로 함께 제공**해줘야 해.
