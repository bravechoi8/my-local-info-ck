...
async function verifyWelfarePopularity(item) {
  try {
    const prompt = `너는 전문 블로그 콘텐츠 기획자야. 아래 공공데이터 혜택 정보가 [전 국민 대상 지원금, 소상공인 금융 대출/수당, 청년 수당/우대통장, K-패스/교통비 환급, 국가/지자체 에너지 보조금, 보육료/아동 장려금, 연말정산/세금 환급, 대다수 서민 주거 대출/보증금 지원] 등 대다수의 일반 시민, 서민, 청년, 직장인, 주부, 소상공인 등이 광범위하게 혜택을 누릴 수 있고 조회수(인기)가 매우 높을 만한 검증된 메이저급 초대형 혜택 정보인지 판단해줘.

지자체나 정부의 극소수 소수 대상(예: 대체초지조성비 감면, 우수숙련기술자 지원, 연탄보일러 보급, 요양원 시설 검사비, 특정 소외 계층 위주 혜택 등)의 마이너하고 대중성(조회수)이 극히 낮은 지엽적 주제라면 절대 승인하지 마세요.

정보: ${JSON.stringify(item)}

대중적이고 전 국민적으로 인기 있을 알짜 핵심 혜택 정보가 맞다면 정확히 'YES', 대다수 대중과 관련 없거나 마이너한 주제라면 'NO'로만 대답해줘. 다른 텍스트는 일체 포함하지 마.`;
    ...
    if (geminiResponse.ok) {
      ...
      const text = (geminiResult.candidates?.[0]?.content?.parts?.[0]?.text || '').trim().toUpperCase();
      return text.includes('YES');
    }
  } catch (err) {
    console.error('[대중성 검증 오류]:', err.message);
  }
  return false;
}
...
async function main() {
  try {
    ...
    for (const item of dataList) {
      // 대중성 있는 복지글은 하루에 딱 1개씩만 쓰도록 엄격히 제한하여 도배 방지
      ...
      if (!isPosted) {
        // Gemini AI를 통한 최종 전 국민 대중성 검증 통과 여부 확인
        console.log(`[AI 대중성 검증 대기] '${itemName}' 혜택 검증 중...`);
        const isPopular = await verifyWelfarePopularity(item);
        if (isPopular) {
          console.log(`[AI 대중성 검증 승인] 전 국민 대박 혜택 매칭: ${itemName}`);
          itemsToPost.push(item);
        } else {
          console.log(`[AI 대중성 검증 반려] 마이너/비인기 혜택 제외: ${itemName}`);
        }
      }
    }
