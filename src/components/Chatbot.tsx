      ...
      setMessages([
        {
          id: 1,
          sender: "bot",
          text: "안녕하세요! '리얼인포'의 친절한 정보 가이드 '척척'입니다. 아래 질문 리스트에서 궁금하신 내용을 선택하시거나, 입력창에 직접 질문을 남겨주시면 자세히 답변해 드릴게요!",
        },
      ]);
    }
  }, [messages]);
      ...
      <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3">
        ...
        {!isOpen && !isTetrisPage && (
          <div className="absolute bottom-16 right-0 bg-white text-[#FF8A00] border border-[#FFEEDC] px-3.5 py-2 rounded-2xl shadow-xl text-xs font-bold whitespace-nowrap flex items-center gap-1.5 animate-pulse z-50">
            ...
            척척 챗봇 💬
            {/* 말풍선 꼬리 (아래쪽을 향하도록 변경) */}
            <div className="absolute bottom-[-6px] right-5 w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-t-[6px] border-t-white"></div>
          </div>
        )}
          ...
          {isLoading && !isHumanMode && (
            <div className="flex items-start gap-2 max-w-[85%]">
              ...
              <div className="bg-white text-[#8B95A1] border border-[#E5E8EB] rounded-2xl rounded-tl-none p-3 text-xs leading-relaxed shadow-sm flex items-center gap-1.5 animate-pulse">
                ...
                <span className="text-[10px] ml-1 text-[#8B95A1] font-semibold">척척 챗봇이 생각하는 중...</span>
              </div>
            </div>
          )}
